...
where
  (
    org.login in (
      'OpenStack-mobile', 'openstack', 'openstack-ansible', 'openstack-charmers', 'openstack-dev', 'openstacknetsdk',
      'openstack-hyper-v', 'openstack-infra', 'openstack-kr', 'openstack-packages', 'openstack-snaps'
    )
  )
  ...
  AND actor.login NOT IN (
    ...
    FROM (
      SELECT
        ...
        COUNT(*) c
      FROM
        TABLE_DATE_RANGE([githubarchive:day.],TIMESTAMP('2017-08-01'),TIMESTAMP('2017-11-01'))WHERE
