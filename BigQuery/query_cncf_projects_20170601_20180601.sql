...
from
  (select * from
    TABLE_DATE_RANGE([githubarchive:day.],TIMESTAMP('2017-06-01'),TIMESTAMP('2018-05-31'))
  )
where
  (
    org.login in (
      ...
      'cloudevents', 'openeventing', 'telepresenceio', 'helm'
    )
    or repo.name in (
      ...
      'rktproject/rkt', 'datawire/telepresence'
    )
  )
  ...
  AND actor.login NOT IN (
    ...
    FROM (
      SELECT
        ...
        COUNT(*) c
      FROM
        TABLE_DATE_RANGE([githubarchive:day.],TIMESTAMP('2017-06-01'),TIMESTAMP('2018-06-31'))
      WHERE
