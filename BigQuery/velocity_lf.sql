...
WHERE
  (
    org.login IN (
      ...
      'theupdateframework', 'rook', 'vitessio', 'telepresenceio', 'helm', 'goharbor', 'kubernetes-csi', 'agones-dev',
      ...
      'devstream-io', 'kubedl-io', 'kubevela', 'konveyor', 'external-secrets', 'krustlet', 'openembedded',
      ...
      'velero-io', 'openeverest', 'nmstate', 'higress-group', 'llm-d', 'thanos-community', 'kube-bind', 'CurvineIO'
    )
    OR repo.name IN (
      ...
      'AbsaOSS/k8gb', 'AbsaOSS/ohmyglb', 'Comcast/trickster', 'datawire/ambassador',
      'vmware-tanzu/antrea', 'cheyang/fluid', 'rancher/submariner', 'alibaba/kubedl',
      ...
      'interTwin-eu/interlink-slurm-plugin', 'interTwin-eu/interlink-docker-plugin', 'NVIDIA/KAI-Scheduler',
      ...
      'vmware-tanzu/velero-csi-plugin', 'percona/everest', 'alibaba/higress', 'Apicurio/apicurio-registry'
    )
  )
  ...
  and (
    type = 'PushEvent' or (
      ...
      and LOWER(actor.login) not in (
        ...
        'katacontainersbot', 'prombot', 'prowbot', 'cdk8s-automation', 'copilot', 'claude', 'codex'
      )
    )
  )
