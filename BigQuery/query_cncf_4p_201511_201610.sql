...
from
  (select * from
    TABLE_DATE_RANGE([githubarchive:day.],TIMESTAMP('2015-11-01'),TIMESTAMP('2016-11-01'))
  )
where
  (
    org.login in (
  ...
  AND actor.login NOT IN (
    ...
    FROM (
      SELECT
        ...
        COUNT(*) c
      FROM
        TABLE_DATE_RANGE([githubarchive:day.],TIMESTAMP('2016-08-01'),TIMESTAMP('2016-11-01'))
      WHERE
