...
Update BigQuery [query file](BigQuery/velocity_top30_without_committers.sql). If a project does not have a GitHub repo or only lists a mirror, skip it for now but later add manually.

Run the query for a year date range, for example: `./run_bq_standard.sh top30_without_committers 20250101 20260101`.
...
- `` ./run_bq_standard.sh top30_without_committers 20250101 20250101; ./run_bq_standard.sh top30_without_committers 20250101 20260101; OUT=data/data_top30_without_committers_projects_20250101_20260101.csv ./merge_bq.rb data/data_top30_without_committers_projects_20250101_20250101.csv data/data_top30_without_committers_projects_20250101_20260101.csv ``.
- Note that it takes many minutes, so it should run in the background and be monitored.

It will generate a file for example: `data/data_top30_without_committers_projects_20250101_20260101.csv` or `data/data_top30_2025.csv`.
...
```
./tools/enrich_authors/enrich_authors -in data/data_top30_without_committers_projects_20250101_20260101.csv -out data/data_top30_without_committers_projects_20250101_20260101.enriched.csv -from 2025-01-01 -to 2026-01-01 -forks all_forks.json
cp data/data_top30_without_committers_projects_20250101_20260101.csv data/data_top30_without_committers_projects_20250101_20260101.raw.csv
cp data/data_top30_without_committers_projects_20250101_20260101.enriched.csv data/data_top30_without_committers_projects_20250101_20260101.csv
```
...
Existing script `shells/unlimited_both.sh` generates our chart data for 2025-01-01 to 2026-01-01. Let's assume we want to generate the chart for a new date range: 2025-01-01 to 2026-01-01. This is a step-by-step tutorial on how to accomplish that.
- Copy `shells/unlimited_both.sh` to `shells/unlimited_20250101-20260101.sh`
- Keep `shells/unlimited_20250101-20260101.sh` opened in some other terminal window `vi shells/unlimited_20250101-20260101.sh` as we need to update all steps. Change all the dates to a new range now so you do not forget and run mixed data.
- First, we need unlimited BigQuery output for a new date range:
```
echo "Restoring BigQuery output"
cp data/data_top30_without_committers_projects_20250101_20260101.csv data/unlimited.csv
```
...
- Go to google storage and download `<<your_google_user_name>>/unlimited_202207_202407.csv` and put it where `shells/unlimited_20250101-20260101.sh` expects it (update the file name to `data/unlimited_output_202207_202407.csv`): 
```
echo "Restoring BigQuery output"
...
- Once You have all URL's defined, added new mapping, you may see a preview of the Top projects on while stopped in `binding.pry`, by typing `all`. Now we need to go back to `shells/unlimited_20250101-20260101.sh` and regenerate all non standard data (for projects not on github or requiring special queries on github - for example because of having 0 activity, comments, commits, issues, prs or authors)
...
- Add a row for the time period in `data/data_linux.csv`: `torvalds,torvalds/linux,2025-01-01,2026-01-01,0,0,0,0,0,0,0,0`
...
- Go to `cncf/gitdm/`, `cd ~/dev/cncf/gitdm/src` and run: `./linux_range.sh 2025-01-01 2026-01-01`
- While in `cncf/gitdm/` directory, view: `vim linux_stats/range_2025-01-01_2026-01-01.txt`:
```
...
```
torvalds,torvalds/linux,2025-01-01,2026-01-01,64482,3790914,1522111,3803,0,0,0,0
```
- Run `PG_PASS=... ./linux_commits.sh 2025-01-01 2026-01-01` that will give values for number of pushes and commits. This is not needed but recommended. Otherwise put `0,0` for commits and pushes. Changesets are used to calculate output commits.
- Run `./lkml_analysis.rb 2025-01-01 2026-01-01` to get number of LKML emails (all) and new threads.
Run this from the velocity project's root folder again:
`ruby add_linux.rb data/data_lf_projects_20250101_20260101.csv data/data_linux.csv 2025-01-01 2026-01-01`


### CNCF Projects case

- We have a line in `` ruby merger.rb data/unlimited.csv data/data_cncf_projects.csv` which needs to be changed to `ruby merger.rb data/unlimited.csv data/data_cncf_projects_20250101_20260101.csv ``.
...
- Go to `cncf/gitdm:src` and run GitLab repo analysis: `./repo_in_range.sh ~/dev/gitlab-ce/ gitlab 2025-01-01 2026-01-01`
- Results are output to `other_repos/gitlab_2025-01-01_2026-01-01.txt`:
```
...
- To count authors run in gitlab-ce directory: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%aE" | sort | uniq | wc -l` --> 575
- To count commits: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%H" | sort | uniq | wc -l` (this will return all possible distinct SHA values, maybe some need to be skipped).
...
- Run `./run_bq_templated.sh cf 20250101 20260101 || echo 'error'` to get Cloud Foundry data. It will generate `data/data_cf_projects_20250101_20260101.csv` file.
- Update (and eventually manually run) the CF case (in `shells/unlimited_20250101-20260101.sh`): `ruby merger.rb data/unlimited.csv data/data_cloudfoundry_202207_202407.csv force`
...
- `cd openstack; PG_PASS=... ./openstack.sh 2025-01-01 2026-01-01 1>/dev/null`
- `./all_openstack.sh 2025-01-01 2026-01-01`.
- `ruby merger.rb data/unlimited.csv openstack/data_openstack_2025-01-01_2026-01-01.csv`.
- New approach: `./openstack_issues.sh '2025-01-01 00:00:00' '2026-01-01 00:00:00'`. Get data from results - it is terribly slow, almost unusable.
- Update file `data/data_openstack_bugs_20250101_20260101.csv` (copy file from `data/data_openstack_bugs.csv`)
...
- The final line should be `ruby update_projects.rb projects/unlimited_both.csv data/data_openstack_bugs_20250101_20260101.csv -1`

### Apache

- Run `./run_bq_templated.sh apache 20250101 20260101 || echo 'error'` to get Apache data. It will generate `data/data_apache_projects_20250101_20260101.csv` file.
- `ruby merger.rb data/unlimited.csv data/data_apache_projects_20250101_20260101.csv`.
- Now we need more data for Apache from their jira, first copy file from previous data range `cp data/data_apache_jira.csv data/data_apache_jira_20250101_20260101.csv`
- New approach (works, but terribly slow): `./apache_jira.sh '2025-01-01 00:00:00' '2026-01-01 00:00:00'` and/or `[REST=1] ./apache_bugzilla.sh '2025-01-01 00:00:00' '2026-01-01 00:00:00'`. `REST=1` can be used once Apache Bugzilla switch to a newer REST API (not yet).
- Final line for Apache should be: `ruby update_projects.rb projects/unlimited_both.csv data/data_apache_jira_20250101_20260101.csv -1`
- Probably no need to run this as it falls outside Top 100.

### Chromium

- Run `./run_bq_templated.sh chromium 20250101 20260101 || echo 'error'` to get Chromium data. It will generate `data/data_chromium_projects_20250101_20260101.csv` file.
- Merge data `ruby merger.rb data/unlimited.csv data/data_chromium_projects_20250101_20260101.csv`.
- Now the manual part: `cp data/data_chromium_bugtracker.csv data/data_chromium_bugtracker_20250101_20260101.csv` (we need to update this file)
...
All issues + opened>2017/6/1 gives 325, so we have: 63565 - 325 = 63240 issues in 2016-07-19 - 2026-01-01
irb> require 'date'; Date.parse('2026-01-01') - Date.parse('2016-07-19') --> 317
irb> Date.parse('2026-01-01') - Date.parse('2025-01-01') --> 365
...
- Authors: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%aE" | sort | uniq | wc -l` gives 1697
- Commits: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%H" | sort | uniq | wc -l` gives 79144 (but this is only FYI, this is way too many, there are bot commits here)
To analyze those commits (also exclude merge and robot commits):
Run while in chromium/src repository:
`git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%aE~~~~%aN~~~~%H~~~~%s" | sort | uniq > chromium_commits_20250101_20260101.csv`
...
Save and quit vim.
Then move the file to: `cncf/velocity`:`data/data_chromium_commits_20250101_20260101.csv`: `mv chromium_commits_20250101_20260101.csv ~/dev/cncf/velocity/data/data_chromium_commits_20250101_20260101.csv`
Then run `ruby commits_analysis.rb data/data_chromium_commits_20250101_20260101.csv map/skip_commits.csv`
...
Update `data/data_chromium_bugtracker_20250101_20260101.csv` accordingly.
- Final line should be `ruby update_projects.rb projects/unlimited_both.csv data/data_chromium_bugtracker_20250101_20260101.csv -1`
- Probably no need to run this as it falls outside Top 100.

### OpenSUSE

- Run `./run_bq_templated.sh opensuse 20250101 20260101 || echo 'error'` to get OpenSure data. It will generate `data/data_opensuse_projects_20250101_20260101.csv` file.
- Run `ruby merger.rb data/unlimited.csv data/data_opensuse_projects_20250101_20260101.csv`.
- Probably no need to run this as it falls outside Top 100.
...
- To get data for some specific data range: `cd agl; DTFROM="2025-01-01" DTTO="2026-01-01" ./run_multirepo_range.sh` ==> `agl.txt`.
```
Processed 7152 csets from 365 developers
```
- 7152 commits and 365 authors.
- To get number of Issues, search Jira (old approach): `https://jira.automotivelinux.org/browse/SPEC-923?jql=created%20%3E%3D%202025-01-01%20AND%20created%20%3C%3D%202026-01-01`
...
- Finally: `ruby merger.rb data/unlimited.csv data/data_agl_projects_20250101_20260101.csv`

### LibreOffice case

- Run `./run_bq_templated.sh libreoffice 20250101 20260101 || echo 'error'` to get LibreOffice data. It will generate `data/data_libreoffice_projects_20250101_20260101.csv` file.
- Run `ruby merger.rb data/unlimited.csv data/data_libreoffice_projects_20250101_20260101.csv`.
- Now git repo analysis:, first copy `cp data/data_libreoffice_git.csv data/data_libreoffice_git_20250101_20260101.csv` and we will update the `data/data_libreoffice_git_20250101_20260101.csv` file
...
- Commits: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%H" | sort | uniq | wc -l`
- Authors: `git log --all --since "2025-01-01" --until "2026-01-01" --pretty=format:"%aE" | sort | uniq | wc -l`
- Put results in: `data/data_libreoffice_git_20250101_20260101.csv` (authors, commits), values will probably be skipped by the updater tool (they are lower than current values gathered so far)
- Issues (old approach):
...
In the browser window, select rows in range, copy, paste into a text file and see row count. --- OR --- Download as csv to `data/data_libreoffice_bugs.csv`, and then count issues with given date range "2025-01-01" --> "2026-01-01" with `ruby count_issues.rb data/data_libreoffice_bugs.csv Opened '2025-01-01 00:00:00' '2026-01-01 00:00:00'`
```
ruby count_issues.rb data/data_libreoffice_bugs.csv Opened 2025-01-01 2026-01-01
Counting issues in 'data/data_libreoffice_bugs.csv', issue date column is 'Opened', range: 2025-01-01T00:00:00+00:00 - 2026-01-01T00:00:00+00:00
Found 7223 matching issues.
```
Update `data/data_libreoffice_git_20250101_20260101.csv` accordingly.
- New approach, use: `./libreoffice_bugzilla.sh '2025-01-01 00:00:00' '2026-01-01 00:00:00'` (terribly slow).
- Final line should be: `ruby update_projects.rb projects/unlimited_both.csv data/data_libreoffice_git_20250101_20260101.csv -1`
- Probably no need to run this as it falls outside Top 100.

### FreeBSD case

- New approach: Run `./run_bq_templated.sh freebsd 20250101 20260101 || echo 'error'` to get FreeBSD data. It will generate `data/data_freebsd_projects_20250101_20260101.csv` file.
- Run `ruby merger.rb data/unlimited.csv data/data_freebsd_projects_20250101_20260101.csv`.
...
- `cp BigQuery/query_apache_projects.sql BigQuery/query_freebsd_projects.sql` and update conditions, run query, download results, put them in `data/data_freebsd_projects20250101_20260101.csv` (if there aren't many rows, just Download as CSV, otherwise: save as table, export to gstorage, download csv)
...
ruby merger.rb data/unlimited.csv data/data_freebsd_projects_20250101_20260101.csv
```
...
- Use `cncf/gitdm/src/freebsd_svn.sh` script to analyse FreeBSD SVN repos with `./freebsd_svn.sh 20250101 20260101`:
```
Revisions:    35927
Authors:      335
```
- Put results here (authors and commits): `./data/data_freebsd_svn_20250101_20260101.csv`
- Go to: `https://docs.freebsd.org/mail/` and estimate number of emails for your period.
- Old approach: Go to [FreeBSD Bugzilla](https://bugs.freebsd.org/bugzilla/buglist.cgi?chfield=%5BBug%20creation%5D&chfieldfrom=2025-01-01&chfieldto=2026-01-01&order=Last%20Changed&query_format=advanced) and get number of bugs in a given period (bugs=issues, prs=issues).
...
- Click search, results will be limited to first 500, click change columns and choose 'Opened' only (it will show ID and Opened then), finally [url](https://bugs.freebsd.org/bugzilla/buglist.cgi?columnlist=opendate&f1=creation_ts&f2=creation_ts&limit=0&o1=greaterthaneq&o2=lessthaneq&order=opendate%2Cchangeddate%2Cbug_status%2Cpriority%2Cassigned_to%2Cbug_id&query_format=advanced&v1=2025-01-01&v2=2026-01-01).
- New approach: `./freebsd_bugzilla.sh '2025-01-01 00:00:00' '2026-01-01 00:00:00'` (terribly slow).
- Put results here (comments=emails/3 (many of them are automatic)): `./data/data_freebsd_svn_20250101_20260101.csv`
- Finally `ruby update_projects.rb projects/unlimited_both.csv ./data/data_freebsd_svn_20250101_20260101.csv`.
- Use the above two values in a copy of this file: `data_freebsd_svn_20250101_20260101.csv`
- Now rerun `shells/unlimited_20250101_20260101.sh` and see FreeBSD's rank along with the remaining final results.
- Probably no need to run this as it falls outside Top 100.
