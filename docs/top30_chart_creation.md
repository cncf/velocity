...
Run the query for a year date range, for example: `./run_bq.sh top30 2023-07-01 2024-07-01`. It takes about 1+TB and costs about $15-$25+.
Or run the query for a specified year, for example: `./run_bq_year.sh top30_year 2023`. It takes about 1+TB and costs about $15-$25+.
...
- `` ./run_bq.sh top30 2023-07-01 2024-01-01; ./run_bq.sh top30 2024-01-01 2024-07-01; OUT=data/data_top30_projects_20230701_20240701.csv ./merge_bq.rb data/data_top30_projects_20230701_20240101.csv data/data_top30_projects_20240101_20240701.csv ``.

It will generate a file for example: `data/data_top30_projects_20230701_20240701.csv` or `data/data_top30_2023.csv`.

Run `analysis.rb` with
```
FORKS_FILE=all_forks.json ruby analysis.rb data/data_top30_projects_20230701_20240701.csv projects/projects_top30_20230701_20240701.csv map/hints.csv map/urls.csv map/defmaps.csv map/skip.csv map/ranges_unlimited.csv
```
...
Existing script `shells/unlimited_both.sh` generates our chart data for 2023-07-01 to 2024-07-01. Let's assume we want to generate the chart for a new date range: 2023-07-01 to 2024-07-01. This is a step-by-step tutorial on how to accomplish that.
- Copy `shells/unlimited_both.sh` to `shells/unlimited_20230701-20240701.sh`
- Keep `shells/unlimited_20230701-20240701.sh` opened in some other terminal window `vi shells/unlimited_20230701-20240701.sh` as we need to update all steps. Change all the dates to a new range now so you do not forget and run mixed data.
...
- Go to google storage and download `<<your_google_user_name>>/unlimited_202207_202307.csv` and put it where `shells/unlimited_20230701-20240701.sh` expects it (update the file name to `data/unlimited_output_202207_202307.csv`): 
```
echo "Restoring BigQuery output"
...
```
- Run `PG_PASS=... ./linux_commits.sh 2023-07-01 2024-07-01` that will give values for number of pushes and commits. This is not needed but recommended. Otherwise put `0,0` for commits and pushes. Changesets are used to calculate output commits.
- Run `./lkml_analysis.rb 2023-07-01 2024-07-01` to get number of LKML emails (all) and new threads.
...
- Go to `cncf/gitdm:src` and run GitLab repo analysis: `./repo_in_range.sh ~/dev/gitlab-ce/ gitlab 2023-07-01 2024-07-01`
...
- Run `./run_bq.sh cf 2023-07-01 2024-07-01 || echo 'error'` to get Cloud Foundry data. It will generate `data/data_cf_projects_20230701_20240701.csv` file.
- Update (and eventually manually run) the CF case (in `shells/unlimited_20230701-20240701.sh`): `ruby merger.rb data/unlimited.csv data/data_cloudfoundry_202207_202307.csv force`


### OpenStack case

- Newer method - use CNCF devstats contrib instance:
- `cd openstack; PG_PASS=... ./openstack.sh 2023-07-01 2024-07-01 1>/dev/null`
- `./all_openstack.sh 2023-07-01 2024-07-01`.
- `ruby merger.rb data/unlimited.csv openstack/data_openstack_2023-07-01_2024-07-01.csv`.
- New approach: `./openstack_issues.sh '2023-07-01 00:00:00' '2024-07-01 00:00:00'`. Get data from results - it is terribly slow, almost unusable.
- Update file `data/data_openstack_bugs_20230701_20240701.csv` (copy file from `data/data_openstack_bugs.csv`)
...
- The final line should be `ruby update_projects.rb projects/unlimited_both.csv data/data_openstack_bugs_20230701_20240701.csv -1`

### Apache

- Run `./run_bq.sh apache 2023-07-01 2024-07-01 || echo 'error'` to get Apache data. It will generate `data/data_apache_projects_20230701_20240701.csv` file.
...
- New approach (works, but terribly slow): `./apache_jira.sh '2023-07-01 00:00:00' '2024-07-01 00:00:00'` and/or `[REST=1] ./apache_bugzilla.sh '2023-07-01 00:00:00' '2024-07-01 00:00:00'`. `REST=1` can be used once Apache Bugzilla switch to a newer REST API (not yet).
...
- Run `./run_bq.sh chromium 2023-07-01 2024-07-01 || echo 'error'` to get Chromium data. It will generate `data/data_chromium_projects_20230701_20240701.csv` file.
...
All issues + opened>2017/6/1 gives 325, so we have: 63565 - 325 = 63240 issues in 2016-07-19 - 2024-07-01
irb> require 'date'; Date.parse('2024-07-01') - Date.parse('2016-07-19') --> 317
irb> Date.parse('2024-07-01') - Date.parse('2023-07-01') --> 365
...
- Commits: `git log --all --since "2023-07-01" --until "2024-07-01" --pretty=format:"%H" | sort | uniq | wc -l` gives 79144 (but this is only FYI, this is way too many, there are bot commits here)
To analyze those commits (also exclude merge and robot commits):
Run while in chromium/src repository:
`git log --all --since "2023-07-01" --until "2024-07-01" --pretty=format:"%aE~~~~%aN~~~~%H~~~~%s" | sort | uniq > chromium_commits_20230701_20240701.csv`
...
Save and quit vim.
Then move the file to: `cncf/velocity`:`data/data_chromium_commits_20230701_20240701.csv`: `mv chromium_commits_20230701_20240701.csv ~/dev/cncf/velocity/data/data_chromium_commits_20230701_20240701.csv`
Then run `ruby commits_analysis.rb data/data_chromium_commits_20230701_20240701.csv map/skip_commits.csv`
...
Update `data/data_chromium_bugtracker_20230701_20240701.csv` accordingly.
...
- Run `./run_bq.sh opensuse 2023-07-01 2024-07-01 || echo 'error'` to get OpenSure data. It will generate `data/data_opensuse_projects_20230701_20240701.csv` file.
...
- Run `./run_bq.sh libreoffice 2023-07-01 2024-07-01 || echo 'error'` to get LibreOffice data. It will generate `data/data_libreoffice_projects_20230701_20240701.csv` file.
...
- New approach, use: `./libreoffice_bugzilla.sh '2023-07-01 00:00:00' '2024-07-01 00:00:00'` (terribly slow).
...
- New approach: Run `./run_bq.sh freebsd 2023-07-01 2024-07-01 || echo 'error'` to get FreeBSD data. It will generate `data/data_freebsd_projects_20230701_20240701.csv` file.
...
- Use `cncf/gitdm/src/freebsd_svn.sh` script to analyse FreeBSD SVN repos with `./freebsd_svn.sh 20230701 20240701`:
```
Revisions:    35927
Authors:      335
```
...
- Old approach: Go to [FreeBSD Bugzilla](https://bugs.freebsd.org/bugzilla/buglist.cgi?chfield=%5BBug%20creation%5D&chfieldfrom=2023-07-01&chfieldto=2024-07-01&order=Last%20Changed&query_format=advanced) and get number of bugs in a given period (bugs=issues, prs=issues).
...
- Now rerun `shells/unlimited_20230701_20240701.sh` and see FreeBSD's rank along with the remaining final results.
