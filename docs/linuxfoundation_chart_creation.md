...
- A message will be shown: `Data range not found in data/data_linux.csv: 2022-07-01 - 2023-07-01`. That means you need to add a new data range for Linux in file: `data/data_linux.csv`
- Add a row for the time period in `data/data_linux.csv`: `torvalds,torvalds/linux,2022-07-01,2023-07-01,0,0,0,0,0,0,0,0`
	...
	```
- Create `devstats-reports` pod, shell into it and run: `./velocity/linux_commits.sh 2022-07-01 2023-07-01` that will give values for number of pushes and commits. This is not needed but recommended. Otherwise put `0,0` for commits and pushes. Changesets are used to calculate output commits.
...
```
FORKS_FILE=lf_forks.json ruby analysis.rb data/data_lf_projects_20220701_20230701.csv projects/projects_lf_20220701_20230701.csv map/hints.csv map/urls.csv map/defmaps.csv map/skip.csv map/ranges_sane.csv
```
...
Currently manually check for `oam-dev`, `layer5io` and `pixie-labs` in `projects/projects_lf_20220701_20230701.csv` file. Also check for last column being empty `/s,""`.
...
Make a copy of the [google doc](https://docs.google.com/spreadsheets/d/1tAGKvM8sbHnP6borE2mrdOs0bA4HJDNl8XmdVnpoAvs/edit?usp=sharing).
